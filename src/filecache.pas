...
type
  ...
  private
    ...
    FAbort: Boolean;
    function AddItem(Name: UnicodeString): PFileCacheItem;
    procedure ClearItem(Item: Pointer);
    procedure ReadFile(p: PFileCacheItem; Filename: UnicodeString); overload;
    function ReadFile(URL, Filename: UnicodeString): PFileCacheItem; overload;
    procedure CompressItem(p: PFileCacheItem);
    procedure Scan(const Dir, BaseDir: UnicodeString);

    procedure ReloadFileIfChanged(var p: PFileCacheItem; target: UnicodeString);
  public
    constructor Create;
    destructor Destroy; override;
    procedure ProcessTick;
    procedure Abort;
    procedure DoScan(const Dir, BaseDir: UnicodeString);
    procedure AddSymlink(source, dest: UnicodeString);
    function FindPathUrl(var target, PathUrl: UnicodeString): PFileCacheItem;
    function Exists(target: UnicodeString): Boolean;
    function Find(target: UnicodeString): PFileCacheItem;
    function Release(Item: PFileCacheItem): Boolean;
    procedure AddFile(target: UnicodeString; const data, filename: UnicodeString);
    function GetFile(target: UnicodeString; var data: UnicodeString; var GZip: Boolean; RangeStart: Integer = 0; RangeEnd: Integer = -1): Integer;
    procedure WriteFile(target: UnicodeString);
...
function TFileCache.AddItem(Name: UnicodeString): PFileCacheItem;
var
  p: PFileCacheItem;
  i: Integer;
begin
...
procedure TFileCache.ReadFile(p: PFileCacheItem; Filename: UnicodeString);
var
  f: File;
begin
  if Assigned(p^.Filedata) then
...
function TFileCache.ReadFile(URL, Filename: UnicodeString): PFileCacheItem;
var
  f: File;
begin
  result:=nil;
...
procedure TFileCache.DoScan(const Dir, BaseDir: UnicodeString);
begin
  //FCS.Enter;
  FAbort:=False;
  try
...
procedure TFileCache.Scan(const Dir, BaseDir: UnicodeString);
var
  i: Integer;
  sr: TSearchRec;
  p: PFileCacheItem;
  fullpath, fakepath, basePath: UnicodeString;
begin
...
procedure TFileCache.ReloadFileIfChanged(var p: PFileCacheItem; target: UnicodeString);
var
  p2: PFileCacheItem;
  t: TDateTime;
begin
...
procedure TFileCache.AddSymlink(source, dest: UnicodeString);
begin
  FCS.Enter;
  try
...
function TFileCache.FindPathUrl(var target, PathUrl: UnicodeString
  ): PFileCacheItem;
var
  i: Integer;
begin
...
function TFileCache.Exists(target: UnicodeString): Boolean;
begin
  result:=Assigned(FFiles[target]);
end;

function TFileCache.Find(target: UnicodeString): PFileCacheItem;
var
  s: AnsiString;
begin
  FCS.Enter;
...
procedure TFileCache.AddFile(target: UnicodeString; const data,
  filename: UnicodeString);
var
  p: PFileCacheItem;
begin
...
function TFileCache.GetFile(target: UnicodeString; var data: UnicodeString;
...
procedure TFileCache.WriteFile(target: UnicodeString);
var
  p: PFileCacheItem;
  f: File;
begin
